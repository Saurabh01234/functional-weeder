...
defmodule Task4CPhoenixServerWeb.ArenaLive do
  ...
  def handle_event("start_clock", data, socket) do
    ...
    Phoenix.PubSub.broadcast_from!(Task4CPhoenixServer.PubSub, self(), "robot:astart", %{"stringa" => socket.assigns.robotA_start, "stringb" => socket.assigns.robotB_start})
    Phoenix.PubSub.broadcast_from!(Task4CPhoenixServer.PubSub, self(), "robot:bstart", %{"stringa" => socket.assigns.robotA_start, "stringb" => socket.assigns.robotB_start})
  ...
  socket = if data["client"]=="robot_A" do
    ...
    img = cond do
      data["face"]=="south" ->
        "robot_facing_south.png"
        data["face"]=="north" ->
          "robot_facing_north.png"
          data["face"]=="east" ->
            "robot_facing_east.png"
            data["face"]=="west" ->
              "robot_facing_west.png"
    end
    socket = assign(socket, :img_robotA, img)
    socket = assign(socket, :robotA_goals, data["ga"])
    socket = assign(socket, :robotB_goals, data["gb"])
    ox=data["ox"]
    oy=data["oy"]
    socket =  if data["obs"] == true do
      socket = assign(socket, :obstacle_pos, map=MapSet.put(socket.assigns.obstacle_pos, {ox, oy}))
    else socket=socket
   end
    ...
    socket = assign(socket, :left_robotB, data["leftb"])
    socket = assign(socket, :bottom_robotB, data["bottomb"])
    socket = assign(socket, :robotA_goals, data["ga"])
    socket = assign(socket, :img_robot_B, img)
    ox=data["oxb"]
    oy=data["oyb"]
    socket = assign(socket, :robotB_goals, data["gb"])
    socket =  if data["obsb"] == true do
      socket = assign(socket, :obstacle_pos, MapSet.put(socket.assigns.obstacle_pos, {ox,oy}))
    else socket=socket
  end
  ...
  #def handle_info(%{event: "op", payload: data, topic: "robot:obs"}, socket) do
  #  x=data["left"]
#  y=data["bottom"]
#  IO.puts("yeeeeeeees")
#  socket = assign(socket, :obstacle_pos, MapSet.put(:obstacle_pos, {x,y}))
#  {:noreply, socket}
#end
  ######################################################
