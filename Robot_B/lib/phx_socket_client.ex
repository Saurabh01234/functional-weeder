defmodule Task4CClientRobotB.PhoenixSocketClient do
  ...
  def connect_server do
    socket_opts = [
      url: "ws://88b0-2401-4900-5aa6-28c3-589c-de49-4d2a-2a2e.ngrok.io:80/socket/websocket"
    ]
  ...
  def send_robot_status(channel, %Task4CClientRobotB.Position{x: x, y: y, facing: facing} = _robot) do
    {:ok, is_obs_ahead}= PhoenixClient.Channel.push(channel, "bnew_msg", %{"client" => "robot_B", "x" => x, "y" => y, "face" => facing})
...
def getstart(channel) do
  ...
  {:ok, %{"stringa" => r1, "stringb" => r2}}=PhoenixClient.Channel.push(channel, "bstart", %{"client" => "robot_B"})

 r2
end
def endit(channel) do
  {:ok, en}=PhoenixClient.Channel.push(channel, "end", %{"client" => "robot_B"})
 end
 def getgoa(channel) do
  {:ok, [_sow, weed]}=PhoenixClient.Channel.push(channel, "give", %{"client" => "robot_B"})
  weed
 end
  ######################################################
